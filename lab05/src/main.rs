...
impl fmt::Display for Student{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "\nName: {}\nPhone: {}\nAge: {}\n", self.name, self.phone, self.age.unwrap_or(0))
    }
}

fn main(){
    // Problema 1
    let students_text = fs::read_to_string(STUDENTS_FILE)
                .expect("No such file for students");
    ...
    for student in students_text.lines(){
        ...
        let temp_student = Student{
            ...
            age: Some(fields[2].trim().parse::<u8>().expect("Couldn't fit age field into u8 (Invalid age)"))
        };
    ...
    println!("Youngest student: {}\nOldest student: {}", youngest_student, oldest_student);
}